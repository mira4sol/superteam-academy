...
describe('onchain-academy', () => {
  ...
  const learner = Keypair.fromSecretKey(
    new Uint8Array(require('../../wallets/learner.json')),
  )
  ...
  const creator = Keypair.fromSecretKey(
    new Uint8Array(require('../../wallets/creator.json')),
  )
  ...
  before(async () => {
    ...
    xpMintKeypair = Keypair.fromSecretKey(
      new Uint8Array(require('../../wallets/xp-mint-keypair.json')),
    )
  ...
  describe('2. Update Config', () => {
    ...
    it('fails with wrong authority', async () => {
      ...
      const imposter = Keypair.fromSecretKey(
        new Uint8Array(require('../../wallets/imposter.json')),
      )
      // failing
  ...
  describe('11. Creator reward threshold', () => {
    ...
    it('creator gets no reward when below threshold', async () => {
      ...
      const enrollment = await program.account.enrollment.fetch(threshEnrollPda)
      expect(enrollment.completedAt).to.not.be.null
  ...
  describe('14. Edge cases', () => {
    ...
    it('course with single lesson', async () => {
      ...
      const enrollment = await program.account.enrollment.fetch(singleEnrollPda)
      expect(enrollment.completedAt).to.not.be.null
