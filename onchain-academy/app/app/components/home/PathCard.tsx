...
import { PathSVGs } from '@/app/components/home/constants'
import type { PathItem } from '@/app/components/home/types'
...
export function PathCard({ path }: PathCardProps) {
  ...
  return (
    <div
      ...
      className={`relative overflow-hidden rounded-[20px] transition-all duration-300 bg-[var(--landing-cream)] ${
        path.featured
          ? 'border-[1.5px] border-[var(--landing-primary)]'
    ...
    >
      {path.featured && (
        <div className='absolute top-0 left-0 right-0 h-0.5 z-[2] bg-gradient-to-r from-[var(--landing-primary)] to-[#52dda0]' />
      )}

      <div className='relative overflow-hidden bg-[var(--landing-secondary)] py-6 px-5 sm:py-8 sm:px-8 pb-6 sm:pb-7'>
        ...
        <h3 className='font-display text-[22px] text-[var(--landing-cream)] mb-2 leading-tight tracking-tight relative z-[1]'>
          {path.title}
        </h3>
        ...
        <div className='flex gap-5 mt-5 pt-4 border-t border-[rgba(247,234,203,0.08)] relative z-[1]'>
          {[
            ...
            <div key={s.label}>
              <div className='text-sm font-bold text-[var(--landing-cream)] font-[inherit] leading-none'>
                {s.value}
              </div>
      ...
      <div className='py-5 px-5 sm:py-6 sm:px-8 pb-6 sm:pb-7 bg-white'>
        ...
        <button
          ...
          className={`w-full py-[11px] rounded-lg text-[13px] font-bold cursor-pointer font-[inherit] flex items-center justify-center gap-2 transition-all ${
            path.featured
              ? 'landing-btn-primary border-none'
              : 'bg-transparent border-[1.5px] border-[rgba(27,35,29,0.2)] text-[var(--landing-charcoal)] hover:border-[var(--landing-primary)] hover:text-[var(--landing-primary)]'
          }`}
        >
