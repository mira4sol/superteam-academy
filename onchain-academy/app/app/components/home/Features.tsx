...
import { FEATURES, FeatureSVGs } from '@/app/components/home/constants'
...
export function Features() {
  ...
  return (
    <section
      id='features'
      aria-label='Features'
      className='bg-[var(--landing-secondary)] py-12 sm:py-16 md:py-24 px-4 sm:px-[5%] relative overflow-hidden'
    >
      ...
      <div className='max-w-[1200px] mx-auto relative z-[1]'>
        <div className='text-center mb-10 sm:mb-14'>
          <div className='text-[11px] font-bold tracking-[0.1em] uppercase text-[var(--landing-accent)] mb-2.5 font-[inherit]'>
            {t('features.sectionLabel')}
          </div>
          <h2 className='font-display text-[clamp(1.75rem,5vw,2.8rem)] text-[var(--landing-cream)] leading-tight mb-3.5'>
        ...
        <div className='border border-[rgba(247,234,203,0.1)] rounded-2xl overflow-hidden grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-0.5'>
          {FEATURES.map((f, i) => {
            ...
            return (
              ...
              >
                {f.accent && (
                  <div className='absolute top-0 left-8 right-8 h-0.5 rounded-b-sm bg-gradient-to-r from-[var(--landing-accent)] to-transparent' />
                )}
                <div className='flex items-start justify-between mb-6'>
                  ...
                  <span
                    className={`font-display text-[13px] font-bold tracking-wide leading-none pt-1 ${f.accent ? 'text-[var(--landing-accent)]' : 'text-[rgba(247,234,203,0.28)]'}`}
                  >
                    {f.num}
                  </span>
                </div>
                <h3 className='font-display text-[19px] text-[var(--landing-cream)] mb-2.5 leading-tight tracking-tight'>
                  {fi.title}
                </h3>
